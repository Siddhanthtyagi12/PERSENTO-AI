...
def test_ip(region, ip):
    ...
    try:
        ...
        conn.close()
        return region, ip, "SUCCESS", None
    except Exception as e:
        error = str(e)
        return region, ip, "FAILED", error
...
if __name__ == "__main__":
    targets = [
        ...
        ("ap-south-1", "3.111.105.85")
    ]
    
    print("Testing connection to all regions via IPv4...")
    with concurrent.futures.ThreadPoolExecutor(max_workers=16) as executor:
        futures = [executor.submit(test_ip, r, i) for r, i in targets]
        
        for future in concurrent.futures.as_completed(futures):
            region, ip, status, error = future.result()
            print(f"Region: {region} ({ip}) -> {status}. Error: {error if error else 'None'}")
