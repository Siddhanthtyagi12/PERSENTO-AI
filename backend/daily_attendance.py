...
def run_attendance(org_id=1):
    ...
    verification_feedback = {} # {id: timestamp} to show "Verified" for X seconds
    
    # MediaPipe Mesh Indices for EAR
    LEFT_EYE = [33, 160, 158, 133, 153, 144]
    RIGHT_EYE = [362, 385, 387, 263, 373, 380]
    EAR_THRESHOLD = 0.18
    RECOGNITION_THRESHOLD = 1.0 # Balanced precision
    STABILITY_FRAMES = 5
            ...
            for face_landmarks in detection_result.face_landmarks:
                ...
                if known_signatures:
                    ...
                    if distances[best_match_idx] < RECOGNITION_THRESHOLD:
                        ...
                        if identity_buffer[match_id] >= STABILITY_FRAMES:
                            ...
                            if blink_counters[match_id]['blinked']:
                                ...
                                if markAttendance(match_id, name, org_id=1):
                                    ...
                                    verification_feedback[match_id] = time.time() # Start feedback timer
                        ...
                        if match_id in verification_feedback:
                            if time.time() - verification_feedback[match_id] < 2.0: # Show for 2 seconds
                                color = (0, 255, 0)
                                status_text = f"{name}: Verified"
                            else:
                                del verification_feedback[match_id]
                    else:
                        color = (0, 0, 255)
                        status_text = "Unknown Person"
                        # REMOVED: identity_buffer = {}  (Don't reset everything on one bad frame)
                else:
                    color = (0, 0, 255)
                ...
                cv2.putText(img, status_text, (min_x, text_y), cv2.FONT_HERSHEY_SIMPLEX, 0.7, color, 2)
